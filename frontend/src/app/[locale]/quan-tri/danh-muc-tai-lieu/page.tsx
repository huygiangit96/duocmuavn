...
async function saveCategory(values: CategoryForm) {
  ...
  if (values.originalSlug) {
    const response = await api.patch<DocCategory>(`/admin/document-categories/${values.originalSlug}/`, payload);
    return response.data;
  }

  const response = await api.post<DocCategory>("/admin/document-categories/", payload);
  return response.data;
}

async function deleteCategory(slug: string) {
  await api.delete(`/admin/document-categories/${slug}/`);
}

export default function AdminDocumentCategoriesPage() {
  const queryClient = useQueryClient();
  const [form, setForm] = useState<CategoryForm | null>(null);
  const { data: categories = [], isLoading } = useQuery({ queryKey: ["admin-document-categories"], queryFn: fetchCategories, retry: false });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["admin-document-categories"] }),
  });
  ...
  const confirmDelete = (category: DocCategory) => {
    if (window.confirm(`Xác nhận xóa danh mục tài liệu ${category.name}?`)) {
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18, gap: 12, flexWrap: "wrap" }}>
        <h2 style={{ margin: 0, fontWeight: 800, fontSize: 22 }}>Danh mục tài liệu</h2>
